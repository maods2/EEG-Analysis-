...
def KNNDWN_model_predict(X_train,X_test,y_train,y_test, scoring=None):
  ...
  knn_model = GridSearchCV(KNN, param_grid, cv=5,scoring=scoring, n_jobs=N_JOBS)
...
def SVM_model_predict(X_train,X_test,y_train,y_test,scoring=None):
  ...
  svm_model = GridSearchCV(svm, param_grid, cv=5,scoring=scoring, n_jobs=N_JOBS)  
  svm_model.fit(X_train, y_train)
  y_pred = svm_model.predict(X_test)
  best_score = round(svm_model.best_score_,3)
  return y_test, y_pred, best_score

def XGB_model_predict(X_train,X_test,y_train,y_test,scoring=None, binary=False):
  ...
  xgb = XGBClassifier(objective=objective, random_state=0)
  xgb_model = GridSearchCV(xgb, param_grid, cv=5,scoring=scoring, n_jobs=N_JOBS)  
...
def LDA_model_predict(X_train,X_test,y_train,y_test,scoring=None):
  ...
  lda_model = GridSearchCV(lda, cv=5,param_grid=param_grid, scoring=scoring, n_jobs=N_JOBS)  
