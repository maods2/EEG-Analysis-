...
def load_eeg_data_edf(directory:str, extension:str, exclusions: List[str]=["02.edf", "01.edf"]):
    p = Path(directory)
    file_name_list = [x for x in p.glob(extension) if not x.name.endswith(tuple(exclusions))]
    features, labels =[], []
    for file_name in file_name_list:
...
def load_eeg_data_mat(directory:str, extension:str= "*.mat", exclusions:List[str]=[]):
    p = Path(directory)
    file_name_list = [x for x in p.glob(extension) if not x.name.endswith(tuple(exclusions))]
    features, labels =[], []
    for file_name in file_name_list:
        try:
            feature, label = read_data_mat(file_name)
...
def read_data_mat(file_path:str):
    ...
    if  "mrk" not in m:
        raise Exception("There is no key called 'mrl' present in files {file_path}.")
